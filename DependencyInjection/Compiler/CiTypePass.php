...
class CiTypePass implements CompilerPassInterface
{
    ...
    public function process(ContainerBuilder $container)
    {
        ...
        if (!$container->has('digip_deploy.ci_type_builder')) {
            return;
        }

        $definition = $container->findDefinition('digip_deploy.ci_type_builder');

        $taggedServices = $container->findTaggedServiceIds('digip_deploy.ci_type');
